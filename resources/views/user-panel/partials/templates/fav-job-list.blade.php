...
$gallery = asset('public/images/placeholder.png');
...
<div class="col-sm-12 pr-0 end_fav_item" data-name="{{$job->name}}">
    <div class="row product-list-item mr-1 p-sm-1 mt-3">
        ...
        <div class="detailed-section @if(Request::is('my-business/favorite-list/*')) col-md-9 @else col-md-8 @endif position-relative p-2">
            ...
            @endif
            <div class="location text-muted mt-2 float-left">{{$job->sector}}</div>
                    ...
                    <div id="note_{{$job->ad ? $job->ad->id : ''}}" class="{{$fav_item && $fav_item->id && !$fav_item->note ? 'collapse' : ''}}" style="background-color: #fff5cb; border-radius:10px">
                        <form class="p-3" id="note_form_{{$job->ad ? $job->ad->id : ''}}">
                            ...
                            <div class="mt-3 float-left d-none remove_button_area">
                                <a href="javascript:void(0)" class="remove_note_button" style="color:red;">Slett</a>
                            </div>

                            <div class="mt-3 float-right {{$fav_item && $fav_item->id && $fav_item->note ? 'd-none' : ''}}">
                                <a class="btn btn-warning btn-sm close_button {{$fav_item && $fav_item->id && $fav_item->note ? 'close_button_for_note' : ''}}" @if($fav_item && $fav_item->id && !$fav_item->note) data-toggle="collapse" data-target="#note_{{$job->ad ? $job->ad->id : ''}}" @endif>Avbryt</a>
                                <input type="submit" value="Lagre" data-target="note_form_{{$job->ad ? $job->ad->id : ''}}" class="btn btn-success btn-sm submit_button">
                            </div>
