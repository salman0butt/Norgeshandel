    ...
    <div class="carousel-inner">
        ...
        @if($name->count() > 0)
            @foreach($name as $key=>$val)
                ...
                $path  =    \App\Helpers\common::getMediaPath($val);
                //dd($path);
                $file_path = 'public/uploads/' . date('Y', strtotime($val->updated_at)) . '/' . date('m', strtotime($val->updated_at)) . '/';
                ...
                <div class="carousel-item <?php echo($i == 0 ? "active" : ""); ?>" style="text-align: center; width:100%;">
                    ...
                    @else
                       <a data-fslightbox="gallery1" href="{{$path}}" style="display: block;width:100%;">
                           <img class="d-block" src="{{$path}}" alt="First slide" style="max-width: 100%;max-height: 800px!important;margin: auto">
                       </a>
                    @endif
